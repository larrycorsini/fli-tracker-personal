...
def get_base_html_head(
    ...
    head = [
        ...
        "        .deal-chip-active {",
        "            background: var(--accent-interactive); color: #fff; border-color: var(--accent-interactive);",
        "        }",
        "        .deal-chip-active:hover { background: var(--accent-interactive-hover); }",
        "        .deal-chip-disabled {",
...
def infer_premium_deal_is_domestic(deal: dict) -> bool:
    """Return whether a premium deal is US domestic (for client-side filters)."""
    if "is_domestic" in deal:
        return bool(deal["is_domestic"])
    if deal.get("type") == "domestic":
        return True
    airport = deal.get("airport", "")
...
def render_premium_deals_section() -> list[str]:
    ...
    return [
        ...
        "                <h3 class='text-xs font-bold uppercase tracking-wider text-gray-500'>Filter deals</h3>",
        "                <p class='text-sm text-gray-500' x-show='filtersActive'",
        "                   x-text=\"'Showing ' + filteredDeals.length + ' of ' + allDeals.length + ' deals'\"></p>",
        "            </div>",
        ...
        "                        <input type='checkbox' class='w-5 h-5 rounded border-gray-300'",
        "                               style='accent-color: var(--accent-interactive)'",
        "                               x-model='domesticOnly' aria-label='Show domestic destinations only'>",
        "                        <span class='text-sm font-medium text-gray-700'>Domestic only</span>",
        "                    </label>",
        "                    <label class='inline-flex items-center gap-2 min-h-[44px] cursor-pointer select-none'>",
        "                        <input type='checkbox' class='w-5 h-5 rounded border-gray-300'",
        "                               style='accent-color: var(--accent-interactive)'",
        "                               x-model='maxOneStop' aria-label='Show flights with at most one stop'>",
        "                        <span class='text-sm font-medium text-gray-700'>Max 1 stop</span>",
        "                    </label>",
        ...
        "        </div>",
        "        <div x-show='!loading && !error && allDeals.length > 0 && filteredDeals.length === 0' x-cloak",
        "             class='card-container bg-white border border-gray-200 p-8 text-center text-gray-500'>",
        "            <p class='mb-4'>No deals match your filters.</p>",
        "            <button type='button' @click='clearFilters()' class='btn-accent focus-ring text-sm'>Clear filters</button>",
        "        </div>",
        "        <div x-show='!loading && !error && filteredDeals.length > 0' x-cloak",
        "             class='card-container bg-white border border-gray-200 overflow-hidden divide-y divide-gray-100'",
        "             role='list' aria-label='Premium flight deals'>",
        "            <template x-for='(deal, i) in filteredDeals' :key=\"'premium-' + i\">",
